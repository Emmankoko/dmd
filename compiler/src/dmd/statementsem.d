...
*/
void lowerArrayAggregate(StaticForeach fs, Scope* sc)
{
    auto aggrfe = fs.aggrfe;
    ...
    if (el.op == EXP.int64)
    {
        ...
        else
        {
            ...
            foreach (i; 0 .. length)
            {
                auto index = new IntegerExp(fs.loc, i, Type.tsize_t);
...
void lowerNonArrayAggregate(StaticForeach fs, Scope* sc)
{
    auto aggrfe = fs.aggrfe;
    ...
    if (tplty) sfe.push(new ExpStatement(fs.loc, tplty.sym));
    ...
    aggrfe = new ForeachStatement(fs.loc, TOK.foreach_, pparams[2], aggr,
